...
public:
	...
	void Update(float deltaTime,  bool isShoot) {
		...
		vec3 gunPos = (camera->GetFront() * 0.2f) + (camera->GetRight() * 0.2f) + (camera->GetUp() * -0.1f) + camera->GetPosition();
		gunModel = mat4(1.0);
		...
		gunModel = scale(gunModel, vec3(0.4));
		gunModel = translate(gunModel, vec3(-0.225, 0.0, -0.225));
		gunModel = rotate(gunModel, radians(-75.0f), vec3(0.0, 1.0, 0.0));
	}
	// 渲染场景
	void Render() {
		dotShader->Bind();
...
private:
	// 加载枪模型
	void LoadGun() {
		gun = new Model("res/model/Gun43.obj");
		dot = new Model("res/model/dot.obj");
	}
	// 加载纹理
	void LoadTexture() {
		diffuseMap = new Texture("res/texture/Gun43.png");
	}
	// 加载着色器
	void LoadShader() {
